   ...
   public:
    BPlusTree(const std::string& name) {
        ...
        if (!leaf_file || !tree_file) {
            ...
            tree_file.read(reinterpret_cast<char*>(&last_internal),
                           sizeof(int));
    ...
    void Insert(const Key& key, const Value& value, int timestamp) {
        Record(Rollback::type::INSERT, timestamp, key, Value());
        if (InsertIfFatherSplit({key, value}, root)) {
            ...
            for (int i = 0; i < m - 1; i++)
                new_brother.key[i] = root.key[m + i];
            root.num = m;
    ...
    void Remove(const Key& key, int timestamp) {
    ...
    void RollBack(int timestamp) {
        for (int i = rec; i > 0; i--) {
            ...
            if (roll.op == Rollback::type::INSERT)
                Remove(roll.key, roll.timestamp);
            else if (roll.op == Rollback::type::REMOVE)
                Insert(roll.key, roll.old_value, roll.timestamp);
            else if (roll.op == Rollback::type::MODIFY)
                Modify(roll.key, roll.old_value, roll.timestamp);
        }
    }
   ...
   private:
    ...
    void Print(const Leaf& leaf) {
        for (int i = 0; i < leaf.num; i++)
            std::cout << leaf.val[i].first << " ";

        std::cout << std::endl;
        for (int i = 0; i < leaf.num; i++)
            std::cout << leaf.val[i].second << " ";
    ...
    bool RemoveIfFatherMerge(const Key& key, Internal& f) {
        if (f.is_leaf) {
            ...
            if (leaf.num < m) {
                ...
                if (pos > 0) {
                    ...
                    if (sibilings_left.num > m) {
                        for (int i = leaf.num - 1; i >= 0; i--)
                            leaf.val[i + 1] = leaf.val[i];
                        leaf.val[0] =
                            sibilings_left.val[sibilings_left.num - 1];
                        sibilings_left.num--;
                        leaf.num++;
                        f.key[pos - 1] =
                            sibilings_left.val[sibilings_left.num - 1].first;
                ...
                if (pos > 0) {
                    ReadLeaf(sibilings_left, f.son[pos - 1]);
                    for (int i = 0; i < leaf.num; i++)
                        sibilings_left.val[sibilings_left.num + i] =
                            leaf.val[i];
                    ...
                    for (int i = pos - 1; i < f.num - 2; i++)
                        f.key[i] = f.key[i + 1];
                    for (int i = pos; i < f.num - 1; i++)
                        f.son[i] = f.son[i + 1];
                    f.num--;
                ...
                if (pos < f.num - 1) {
                    ...
                    for (int i = pos; i < f.num - 2; i++)
                        f.key[i] = f.key[i + 1];
                    for (int i = pos + 1; i < f.num - 1; i++)
                        f.son[i] = f.son[i + 1];
                    f.num--;
        ...
        if (RemoveIfFatherMerge(key, son)) {
            ...
            if (pos > 0) {
                ...
                if (sibilings_left.num > m) {
                    for (int i = son.num - 1; i >= 0; i--)
                        son.son[i + 1] = son.son[i];
                    for (int i = son.num - 2; i >= 0; i--)
                        son.key[i + 1] = son.key[i];
    ...
    void WriteInternal(const Internal& internal) {
        ...
        if (ret.first) {
            ...
            tree_file.write(reinterpret_cast<char*>(&ret.second),
                            sizeof(Internal));
        }
    }
    ...
    void ReadInternal(Internal& internal, int pos) {
        ...
        if (ret.first)
            ...
            tree_file.read(reinterpret_cast<char*>(&internal),
                           sizeof(Internal));
        }
    }
